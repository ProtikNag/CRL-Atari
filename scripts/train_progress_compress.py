"""
Train a Knowledge Base via Progress & Compress (Schwarz et al., ICML 2018).

Two-phase sequential continual learning:
  Progress: a fresh Active Column (AC) with lateral KB connections is trained
            on the new task via DQN. KB weights are frozen.
  Compress: KB is fine-tuned to match AC policy via KL distillation, with
            Online EWC regularisation protecting previously acquired skills.

The final saved model is the KB only (no task identifier at inference).

Usage:
    python scripts/train_progress_compress.py [--debug] [--tag default]
    python scripts/train_progress_compress.py --device cuda --tag pc_run1
"""

import argparse
import os
import sys
import subprocess
import torch

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from src.baselines.progress_compress import ProgressCompressTrainer
from src.data.atari_wrappers import compute_union_action_space
from src.utils.config import get_effective_config, save_config
from src.utils.seed import set_seed
from src.utils.logger import setup_logger


def main():
    parser = argparse.ArgumentParser(
        description="Train Knowledge Base via Progress & Compress."
    )
    parser.add_argument(
        "--config", type=str, default="configs/base.yaml", help="Config file."
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode (fast training)."
    )
    parser.add_argument(
        "--device", type=str, default=None, help="Force device: cpu, cuda, mps."
    )
    parser.add_argument(
        "--tag", type=str, default="default", help="Experiment tag."
    )
    args = parser.parse_args()

    # ── Config ───────────────────────────────────────────────────────────
    config = get_effective_config(args.config, debug=args.debug)

    if args.device:
        device = args.device
    elif torch.cuda.is_available():
        device = "cuda"
    elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        device = "mps"
    else:
        device = "cpu"

    set_seed(config["seed"])

    # ── Logger ───────────────────────────────────────────────────────────
    logger = setup_logger(
        log_dir=config["logging"]["log_dir"],
        experiment_name=f"baseline_pc_{args.tag}",
        use_tensorboard=config["logging"]["use_tensorboard"],
        use_wandb=config["logging"]["use_wandb"],
    )

    logger.info(f"Device: {device}")
    logger.info(f"Debug mode: {config['debug']['enabled']}")
    logger.info(f"Task sequence: {config['task_sequence']}")

    pc_cfg = config.get("progress_compress", {})
    debug_cfg = config.get("debug", {})
    debug_on = debug_cfg.get("enabled", False)

    progress_steps = (
        debug_cfg.get("pc_progress_steps", 10000)
        if debug_on
        else pc_cfg.get("progress_steps", 3_000_000)
    )
    compress_epochs = (
        debug_cfg.get("pc_compress_epochs", 5)
        if debug_on
        else pc_cfg.get("compress_epochs", 10000)
    )
    logger.info(f"Progress steps/task: {progress_steps}")
    logger.info(f"Compress epochs/task: {compress_epochs}")

    # ── Union action space ───────────────────────────────────────────────
    union_actions = compute_union_action_space(config["task_sequence"])
    config["model"]["unified_action_dim"] = len(union_actions)
    logger.info(
        f"Union action space: {union_actions} ({len(union_actions)} actions)"
    )

    # Git hash
    try:
        git_hash = subprocess.check_output(
            ["git", "rev-parse", "--short", "HEAD"], stderr=subprocess.DEVNULL
        ).decode().strip()
        logger.info(f"Git commit: {git_hash}")
    except Exception:
        pass

    # Save config
    config_path = os.path.join(
        config["logging"]["log_dir"],
        f"baseline_pc_{args.tag}",
        "effective_config.yaml",
    )
    save_config(config, config_path)

    # ── Train ────────────────────────────────────────────────────────────
    trainer = ProgressCompressTrainer(
        config=config,
        union_actions=union_actions,
        device=device,
        logger=logger,
        tag=args.tag,
    )

    kb = trainer.run(task_sequence=config["task_sequence"])

    # ── Summary ──────────────────────────────────────────────────────────
    logger.info("\n" + "=" * 60)
    logger.info("Progress & Compress Training Complete")
    logger.info("=" * 60)

    ckpt_dir = os.path.join(config["logging"]["checkpoint_dir"], args.tag)
    final_path = os.path.join(ckpt_dir, "consolidated_pc.pt")
    logger.info(f"Final KB model: {final_path}")
    logger.info(
        "Evaluate with:\n"
        f"  python scripts/evaluate.py --model-path {final_path} --all-tasks"
    )

    logger.close()


if __name__ == "__main__":
    main()
