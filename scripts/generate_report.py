...
<nav class="toc">
    ...
    <ol>
        ...
        <li><a href="#sec-action">Unified Action Space and Action Masking</a></li>
        ...
        <li><a href="#sec-init">Global-to-Local Initialization</a></li>
...
<section id="sec-overview">
    ...
    <ol>
        <li><strong>Expert Training</strong> &mdash; Train separate DQN agents, one per Atari game
            (Pong, Breakout, SpaceInvaders). Each expert shares a unified 18-action output head
            covering the full Atari joystick action space, with game-specific action masking.</li>
    ...
    <ul>
        <li><strong>Unified 18-action head</strong> &mdash; All experts output Q-values for all 18 Atari actions.
            Invalid actions are masked to &minus;&infin;. This allows direct weight sharing and averaging.</li>
        <li><strong>Global-to-local initialization</strong> &mdash; Experts are initialized from a shared global
            model (initially random, then updated as the running average of trained experts).</li>
        <li><strong>Q-value normalization</strong> &mdash; Running statistics normalize Q-values to handle scale
            differences across games (Pong rewards in [&minus;1, 1] vs. SpaceInvaders in [0, 500+]).</li>
...
<section id="sec-architecture">
    ...
    <div class="flowchart">
        <!-- Row 1: Config + Global Init -->
        <div class="flow-row">
            <div class="flow-box bg-gold">Config<small>base.yaml</small></div>
            <div class="flow-box bg-blue">Global Model<small>Random Init</small></div>
            <div class="flow-box bg-green">Seed<small>Reproducibility</small></div>
        </div>

        <div class="flow-arrow">&darr;</div>
        <div class="flow-label">Global-to-Local Initialization</div>
        ...
        <!-- Row 3: Parameter Average -->
        <div class="flow-row">
            <div class="flow-box bg-teal" style="min-width: 320px;">
                Parameter Average
                <small>w<sub>global</sub> = (1/T) &middot; &Sigma; w<sub>expert_t</sub></small>
            </div>
        </div>

        <div class="flow-arrow">&darr;</div>

        <!-- Row 4: Consolidation Methods -->
        <div class="flow-row">
...
<section id="sec-dqn">
    ...
    connected layers. The key architectural choice is the <strong>unified 18-action output head</strong>
    that covers the full Atari joystick action space across all games.</p>

    <h3>Architecture</h3>
<pre><code>Input:  (batch, 4, 84, 84)    # 4 stacked grayscale frames, 84&times;84 pixels

Layer 1: Conv2d(4 &rarr; 64,  kernel=8&times;8, stride=4, padding=0) + ReLU
         Output: (batch, 64, 20, 20)

Layer 2: Conv2d(64 &rarr; 128, kernel=4&times;4, stride=2, padding=0) + ReLU
         Output: (batch, 128, 9, 9)

Layer 3: Conv2d(128 &rarr; 128, kernel=3&times;3, stride=1, padding=0) + ReLU
         Output: (batch, 128, 7, 7)

Flatten: (batch, 128 &times; 7 &times; 7) = (batch, 6272)

FC:      Linear(6272 &rarr; 1024) + ReLU

Output:  Linear(1024 &rarr; 18)   # Q-values for all 18 actions</code></pre>

    <p><strong>Total parameters:</strong> ~6.7 million</p>

    <h3>Dueling DQN (optional)</h3>
    <p>Instead of a single output head, splits into two streams:</p>
    <ul>
        <li><strong>Value stream:</strong> Linear(1024 &rarr; 512) &rarr; Linear(512 &rarr; 1)</li>
        <li><strong>Advantage stream:</strong> Linear(1024 &rarr; 512) &rarr; Linear(512 &rarr; 18)</li>
    </ul>
    <div class="equation">
        Q(s, a) = V(s) + A(s, a) &minus; mean<sub>a'</sub> A(s, a')
    </div>

    <h3>Why 18 Actions?</h3>
    <p>The full Atari joystick has 18 possible actions (combinations of directions + fire).
    Each game uses a subset:</p>
    <table>
        <tr><th>Game</th><th>Actions Used</th><th>Indices</th></tr>
    ...
    <p>By outputting all 18, we can directly average or merge expert weights without
    architectural mismatches. Invalid actions are masked to &minus;&infin;.</p>
</section>
...
<section id="sec-action">
    <h2>4. Unified Action Space and Action Masking</h2>
    ...
    <table>
        ...
        <tr><td>0</td><td>NOOP</td><td>6</td><td>UPRIGHT</td><td>12</td><td>RIGHTFIRE</td></tr>
        <tr><td>1</td><td>FIRE</td><td>7</td><td>UPLEFT</td><td>13</td><td>UPRIGHTFIRE</td></tr>
        <tr><td>2</td><td>UP</td><td>8</td><td>DOWNRIGHT</td><td>14</td><td>UPLEFTFIRE</td></tr>
        <tr><td>3</td><td>RIGHT</td><td>9</td><td>DOWNLEFT</td><td>15</td><td>DOWNRIGHTFIRE</td></tr>
        <tr><td>4</td><td>LEFT</td><td>10</td><td>UPFIRE</td><td>16</td><td>DOWNLEFTFIRE</td></tr>
        <tr><td>5</td><td>DOWN</td><td>11</td><td>DOWNFIRE</td><td>17</td><td>UPLEFTFIRE</td></tr>
    </table>

    <h3>Action Masking</h3>
    <p>During action selection, the agent computes Q(s, a) for all 18 actions, then applies:</p>
    <div class="equation">
    ...
    <h3>Action Mapping (UnifiedActionWrapper)</h3>
    <p>The ALE environment expects <em>local</em> action indices (0 to n_game_actions &minus; 1),
    not unified indices. The <code>UnifiedActionWrapper</code> translates:</p>
<pre><code>Example for Pong (minimal actions = [0, 1, 3, 4, 11, 12]):
    unified action 0  &rarr; local index 0
    unified action 1  &rarr; local index 1
    unified action 3  &rarr; local index 2
    unified action 4  &rarr; local index 3
    unified action 11 &rarr; local index 4
    unified action 12 &rarr; local index 5</code></pre>
    <p>This wrapper is the <strong>innermost</strong> wrapper (applied right after <code>gym.make</code>)
    so all other wrappers see the standard local action space.</p>

    <h3>Epsilon-Greedy Exploration</h3>
    <p>During training, with probability &epsilon;, the agent selects a random action
    uniformly from the valid action set (not all 18 actions):</p>
    <div class="equation">
...
<section id="sec-preproc">
    ...
    <ol>
        ...
        <li><code>UnifiedActionWrapper</code> &mdash; Unified &rarr; local action mapping</li>
        ...
        <li><code>ClipRewardEnv</code> &mdash; Clip rewards to &#123;&minus;1, 0, +1&#125; via <code>np.sign</code></li>
    ...
    <div class="callout">
        <strong>Why this order matters:</strong> <code>UnifiedActionWrapper</code> must be innermost
        because it translates actions before any other wrapper processes them. <code>EpisodicLifeEnv</code>
    ...
    Capacity: 100,000 transitions (default) or 1,000 (debug mode). Sampling: random uniform.
...
<section id="sec-training">
    ...
    <ul>
        ...
        <li><strong>Target network</strong> &mdash; Hard-copied from policy every <code>target_update_freq=1000</code> steps</li>
    </ul>
    ...
    <h3>Checkpointing</h3>
    <ul>
        <li>Best model (by evaluation reward) saved automatically</li>
        <li>Periodic checkpoints every <code>save_freq=50,000</code> steps</li>
        <li>Final model saved at end of training</li>
...
<section id="sec-norm">
    ...
    <h3>Method 1: RunningNormalizer (Default)</h3>
    <p>Uses exponential moving average to track per-action running statistics:</p>
    <div class="equation">
        &mu;(t) = (1 &minus; m) &middot; &mu;(t&minus;1) + m &middot; mean(Q<sub>batch</sub>)<br>
        &sigma;(t) = (1 &minus; m) &middot; &sigma;(t&minus;1) + m &middot; var(Q<sub>batch</sub>)
    </div>
    <div class="equation">
        Q&#770; = clamp( (Q &minus; &mu;) / &radic;(&sigma;&sup2; + 10<sup>&minus;8</sup>), &minus;C, C )
    </div>
    <p>Parameters: momentum m = 0.01, clip range C = 10.0.</p>

    <h3>Method 2: PopArtNormalizer (Optional)</h3>
    <p><em>&ldquo;Preserving Outputs Precisely while Adaptively Rescaling Targets&rdquo;</em>
    (van Hasselt et al., 2016). Instead of normalizing Q-values externally, it modifies
    the output layer weights to preserve the network's predictions while changing the
    ...
    <div class="callout success">
        <strong>Significance for consolidation:</strong> In knowledge distillation, we normalize
        Q-values per task before computing KL divergence. This ensures the distillation loss
        treats all games equally regardless of their raw Q-value scale.
    </div>
</section>

<!-- ═══════════════════════════════════════════════════════════
     8. GLOBAL-TO-LOCAL INITIALIZATION
     ═══════════════════════════════════════════════════════════ -->
<section id="sec-init">
    <h2>8. Global-to-Local Initialization</h2>

    <p>A key component of the hierarchical training pipeline. Instead of initializing each
    expert from scratch (random weights), we initialize from a shared &ldquo;global&rdquo; model.</p>

    <h3>Procedure</h3>
    <ol>
        <li>Create a random global model: w<sub>global</sub> ~ Random</li>
        <li><strong>Expert 1 (Pong):</strong>
            <ul>
                <li>w<sub>Pong</sub> := w<sub>global</sub> (initialize from global)</li>
                <li>Train w<sub>Pong</sub> on Pong for T steps</li>
                <li>w<sub>global</sub> := w<sub>Pong</sub></li>
            </ul>
        </li>
        <li><strong>Expert 2 (Breakout):</strong>
            <ul>
                <li>w<sub>global</sub> := mean(w<sub>Pong</sub>, w<sub>global</sub>)</li>
                <li>w<sub>Breakout</sub> := w<sub>global</sub></li>
                <li>Train w<sub>Breakout</sub> on Breakout for T steps</li>
                <li>w<sub>global</sub> := mean(w<sub>Pong</sub>, w<sub>Breakout</sub>)</li>
            </ul>
        </li>
        <li><strong>Expert 3 (SpaceInvaders):</strong>
            <ul>
                <li>w<sub>global</sub> := mean of all prior experts + global</li>
                <li>w<sub>SpaceInv</sub> := w<sub>global</sub></li>
                <li>Train w<sub>SpaceInv</sub> on SpaceInvaders for T steps</li>
            </ul>
        </li>
    </ol>
    ...
    <ul>
        <li>Experts start from a shared representation, making their weights more &ldquo;compatible&rdquo;
            for later merging/consolidation.</li>
        <li>Later experts benefit from features learned by earlier experts (positive transfer).</li>
        <li>The unified output head means all 18 action Q-values get reasonable initialization.</li>
        <li>This is particularly important for HTCL, where the closed-form Taylor update assumes
            expert weights are &ldquo;close&rdquo; to the global model. Starting from global ensures the
            drift &delta;<sub>d</sub> = w<sub>expert</sub> &minus; w<sub>global</sub> is small.</li>
    </ul>
</section>
...
<section id="sec-ewc">
    ...
    <p>Starting from the parameter-averaged global model, optimize:</p>
    <div class="equation">
...
<pre><code>Q_teacher = frozen expert_t(states)  # shape: (batch, 18)
Q_student = global_model(states)     # shape: (batch, 18)</code></pre>
...
<section id="sec-lambda">
    ...
    <p>After the initial Taylor update for task <em>t</em>, we perform <code>catch_up_iterations=2</code>
    refinement steps:</p>
    <ol>
...
<pre><code>for task_t in tasks:
    ...
    <div class="callout success">
        ...
        With 2 catch-up iterations, we achieve significantly better convergence in practice,
...
<section id="sec-hyperparams">
    ...
    <table>
        ...
        <tr><td class="category cat-model" rowspan="4">Model</td>
            <td>Conv channels</td><td>[64, 128, 128]</td><td>same</td></tr>
        <tr><td>FC hidden</td><td>1024</td><td>same</td></tr>
        <tr><td>Unified actions</td><td>18</td><td>same</td></tr>
        <tr><td>Total params</td><td>~6.7M</td><td>same</td></tr>
        ...
        <tr><td>Buffer size</td><td>100,000</td><td>1,000</td></tr>
        <tr><td>Eps decay steps</td><td>250,000</td><td>same</td></tr>
        <tr><td>Target update freq</td><td>1,000</td><td>same</td></tr>
        <tr><td>Gradient clip</td><td>10.0</td><td>same</td></tr>

        <tr><td class="category cat-norm" rowspan="3">Normalization</td>
            <td>Method</td><td>running_stats</td><td>same</td></tr>
        <tr><td>Momentum</td><td>0.01</td><td>same</td></tr>
        <tr><td>Clip range</td><td>10.0</td><td>same</td></tr>
        ...
        <tr><td>Fisher samples</td><td>2,000</td><td>100</td></tr>
        <tr><td>Catch-up iters</td><td>2</td><td>same</td></tr>
